...
def extract_url_features(url):
    ...
    # f56: Known malicious IPs (placeholder)
    try:
...
# ---------------------------
# Helper Function to Process Files and Write CSV
# ---------------------------
def process_feature_extraction(whitelist_path: str, blacklist_path: str, output_csv: str):
    totalfeat = []
    ...
    if totalfeat:
        try:
            ...
            logging.info(f"CSV successfully written to {output_csv}")
        except Exception as e:
...
async def process_files(whitelist: UploadFile = File(...), blacklist: UploadFile = File(...)):
    # Create a unique job id to store this processing instance
    job_id = str(uuid.uuid4())
    job_dir = os.path.join("jobs", job_id)
    os.makedirs(job_dir, exist_ok=True)
    
    # Save the uploaded files
    whitelist_path = os.path.join(job_dir, "Whitelist.txt")
    blacklist_path = os.path.join(job_dir, "Blacklist.txt")
    ...
    output_csv = os.path.join(job_dir, "FeaturesColumn.csv")
    
    try:
        # This call is synchronous. For very long processes, consider using BackgroundTasks.
        process_feature_extraction(whitelist_path, blacklist_path, output_csv)
    except Exception as e:
        logging.error(f"Error processing files for job {job_id}: {e}")
        raise HTTPException(status_code=500, detail="Error processing files.")
    
    # Return the CSV file as a download response
    if os.path.exists(output_csv):
        return FileResponse(output_csv, filename=f"FeaturesColumn_{job_id}.csv", media_type="text/csv")
    else:
